...
OverlappedIOContext::OverlappedIOContext(NetworkSession* session, IOType ioType)
	: sessionObject(session), ioType(ioType)
{
...
NetworkSession::NetworkSession()
	: connected(0), refCount(0), gameRoomNumber(-1), readyStatus(false), sessionType(SessionType::SESSION_TYPE_HEADER)
{
...
void NetworkSession::ResetSession()
{
	gameRoomNumber = -1;
	identifyStr.clear();
	readyStatus = false;
...
bool NetworkSession::PostAccept()
{
...
void NetworkSession::AcceptCompletion()
{
...
bool NetworkSession::Connect(const char* ipAddr, unsigned int port, SessionType type)
{
	CriticalSectionSync sync(cs);
...
void NetworkSession::ConnectCompletion()
{
	CriticalSectionSync sync(cs);
...
bool NetworkSession::ConvertToConnectionSession()
{
	CriticalSectionSync sync(cs);
...
void NetworkSession::DisconnectRequest(DisconnectReason dr)
{
...
void NetworkSession::DisconnectCompletion(DisconnectReason dr)
{
...
void NetworkSession::RecvCompletion(DWORD transferred)
{
	CriticalSectionSync sync(cs);
...
void NetworkSession::SendCompletion(DWORD transferred)
{
	CriticalSectionSync sync(cs);
...
void NetworkSession::ReleaseRef()
{
	CriticalSectionSync sync(cs);
...
bool NetworkSession::AddStream(const Stream& stream)
{
	CriticalSectionSync sync(cs);
...
unsigned short NetworkSession::GetPort()
{
	CriticalSectionSync sync(cs);

	return ntohs(sessionAddress.sin_port);
}

int NetworkSession::GetRoomNumber() { return gameRoomNumber; }

void NetworkSession::SetRoomNumber(int number)
{
	CriticalSectionSync sync(cs);
...
void NetworkSession::SetName(const char* name) 
{
	CriticalSectionSync sync(cs);
...
void NetworkSession::SetReadyState(bool state)
{
	CriticalSectionSync sync(cs);
...
void NetworkSession::ProcessForDisconnect()
{
	CriticalSectionSync sync(cs);

	GameRoom* gettedRoom = iocpManager->GetGameRoomMgr()->GetGameRoomWithNumber(gameRoomNumber);

	if (gameRoomNumber != -1)
	{
		if (gettedRoom->isPlaying())
			gettedRoom->CleanUpForDisconnection(this);
		else
			gettedRoom->LeaveRoom(this);
	}
